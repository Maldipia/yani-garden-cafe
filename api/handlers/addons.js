// ── api/handlers/addons.js ────────────────────────────────────────────────────────────────
// Handles: getAddons, getAddonsAdmin, saveAddon, deleteAddon
...
export async function handle_addons(action, ctx) {
  const { body, req, res, jwtUser, checkAuth, supa, supaFetch, auditLog,
    ...
    if (action === 'getAddons') {
      ...
      res.status(200).json({ ok: r.ok, addons: r.data || [] }); return true;
    }
    ...
    if (action === 'saveAddon') {
      ...
      res.status(200).json({ ok: true, addon: method === 'POST' ? r.data?.[0] : row }); return true;
    }
    ...
    if (action === 'deleteAddon') {
      ...
      res.status(200).json({ ok: r.ok }); return true;
    }
  ...
  const _handled = ['getAddons', 'getAddonsAdmin', 'saveAddon', 'deleteAddon'];
