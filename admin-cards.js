...
var _txnCurrentCard=''; async function openCardTxns(cardNumber){ _txnCurrentCard=cardNumber;
  ...
  try{
    ...
    txns.forEach(function(t,i){
      ...
      html+='<tr style="background:'+bg+';border-bottom:1px solid var(--mist)">'        +'<td style="padding:7px 9px;white-space:nowrap;color:var(--timber)">'+ds+'</td>'        +'<td style="padding:7px 9px"><span style="background:'+(tc[t.type]||'#374151')+';color:#fff;border-radius:10px;padding:2px 8px;font-size:.7rem;font-weight:700">'+_esc(t.type)+'</span></td>'        +'<td style="padding:7px 9px;font-weight:700">₱'+parseFloat(t.amount||0).toFixed(2)+'</td>'        +'<td style="padding:7px 9px;color:#065F46;font-weight:700">'+(disc>0?'₱'+disc.toFixed(2):'—')+'</td>'        +'<td style="padding:7px 9px">₱'+parseFloat(t.balance_before||0).toFixed(2)+'</td>'        +'<td style="padding:7px 9px;font-weight:700">₱'+parseFloat(t.balance_after||0).toFixed(2)+'</td>'        +'<td style="padding:7px 9px;font-size:.72rem">'+(t.order_id?'<a href="javascript:void(0)" onclick="jumpToOrder(&quot;'+_esc(t.order_id)+'&quot;)" style="color:var(--forest-deep);font-weight:700;text-decoration:none;border-bottom:1.5px solid var(--forest-deep)">'+_esc(t.order_id)+'</a>':'—')+'</td>'        +'<td style="padding:7px 9px;font-size:.72rem;color:var(--timber)">'+_esc(t.performed_by||'—')+'</td>'        +'<td style="padding:4px 9px">'+(t.reversed_by_txn          ?'<span style="color:#9CA3AF;font-size:.68rem;font-style:italic">Voided</span>'          :(canVoid?('<button onclick="_voidTxn(&quot;'+_esc(t.id)+'&quot;,&quot;'+_esc(_txnCurrentCard)+'&quot;)" style="padding:4px 10px;background:#B5443A;color:#fff;border:none;border-radius:6px;font-size:.72rem;font-weight:700;cursor:pointer">Void</button>'):''))        +'</td>'        +'</tr>';
    });;
