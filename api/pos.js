...
// ── Handler modules (one per domain) ────────────────────────────────────────
...
import { handle_online }       from './handlers/online.js';
import { handle_tables }       from './handlers/tables.js';
import { handle_reservations } from './handlers/reservations.js';
...
const ALLOWED_ORIGINS = (process.env.ALLOWED_ORIGINS || 'https://pos.yanigardencafe.com,https://yanigardencafe.com,https://admin.yanigardencafe.com').split(',').map(s => s.trim());
  ...
  try {
    ...
    const ctx = {
      ...
      supa, supaFetch, auditLog, getSetting,
      SUPABASE_URL, SUPABASE_KEY, SERVICE_CHARGE_RATE, ORDER_PREFIX,
    };

    const handlers = [
      handle_menu, handle_orders, handle_payments, handle_auth,
      handle_online, handle_tables, handle_reservations,
      handle_inventory, handle_addons, handle_refunds, handle_cash,
    ];
