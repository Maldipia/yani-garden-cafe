...
export async function routeAdminOps(action, body, auth, req, res) {
  // ── GUARD: these actions are owned by dedicated handlers (payments.js for
  // login, admin.js for HR, customers.js, orders.js). This file retains older
  // copies of that logic; bail immediately so it can never shadow the current
  // handlers even if the router dispatch order changes.
  const NOT_OURS = new Set([
    'verifyUserPin', 'changePin',
    'getHRStaff','getHRProfile','updateHRStaff','addHRStaff',
    'getHRLoans','addHRLoan','updateHRLoan','getHRDocuments','addHRDocument',
    'getHRIncidents','addHRIncident','getHRPerformance','addHRPerformance',
    'getHRLeave','getHRTimeLogs','addHRLeaveRequest','addHRTimeLog',
    'hrLookupStaff','hrVerifyPin','hrClockEvent','hrEmployeeLogin',
    'hrCompute13thMonth','hrGetHolidays','hrSetPin','hrSetPortalPin','hrRotateQrToken',
    'getCustomers','getCustomer','upsertCustomer','updateCustomerStats',
    'getTables','getTableStatus',
  ]);
  if (NOT_OURS.has(action)) return false;
