// ── api/handlers/inventory.js ────────────────────────────────────────────────────────────────
// Handles: getInventory, uploadInventoryPhoto, upsertInventory, adjustInventory, getInventoryLog
...
export async function handle_inventory(action, ctx) {
  const { body, req, res, jwtUser, checkAuth, supa, supaFetch, auditLog,
    ...
    if (action === 'uploadInventoryPhoto') {
      ...
      if (!uploadResp.ok) {
        ...
        res.status(500).json({ ok: false, error: 'Upload failed: ' + errText }); return true;
      }
      ...
      res.status(200).json({ ok: true, photoUrl, filename }); return true;
    }
    ...
    if (action === 'upsertInventory') {
      ...
      res.status(200).json({ ok: true, item: r.data?.[0] || row }); return true;
    }
    ...
    if (action === 'adjustInventory') {
      ...
      await supaFetch(`${SUPABASE_URL}/rest/v1/inventory_log`, { method: 'POST',
        body: JSON.stringify({ item_code: itemCode, change_type: type,
          ...
          reference: reference || '' }) });
      res.status(200).json({ ok: true, itemCode, qtyBefore: current.stock_qty, qtyAfter: newQty, direction: qty >= 0 ? 'IN' : 'OUT' }); return true;
    }
    ...
    if (action === 'getInventoryLog') {
      ...
      res.status(200).json({ ok: r.ok, logs: r.data || [] }); return true;
    }
  ...
  const _handled = ['getInventory', 'uploadInventoryPhoto', 'upsertInventory', 'adjustInventory', 'getInventoryLog'];
