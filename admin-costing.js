    ...
    if (rec) {
      ...
      var ingRows = (rec.ingredients || []).map(function(ri, idx) {
        ...
        return '<div style="margin-bottom:8px">'
          ...
          + '</div>'
          + '<input type="number" step="0.1" value="'+ri.qty+'" id="qs-'+rec.id+'-'+idx+'" onchange="updateRecipeIng('+rec.id+','+idx+',\'qty\',this.value)" onkeydown="if(event.key===\'Enter\')this.blur()" style="padding:6px 8px;font-size:.72rem;border:1.5px solid var(--mist);border-radius:var(--r-sm);text-align:right;width:100%">'
